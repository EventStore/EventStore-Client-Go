...
func (self grpcClientImpl) GetConnectionHandle() (ConnectionHandle, error) {
...
func (self connectionHandle) Id() uuid.UUID {
	return self.id
}

func (self connectionHandle) Connection() *grpc.ClientConn {
...
func (msg unexpectedException) handle(state *connectionState) {
	if msg.correlation == state.correlation {
		log.Printf("[error] unexpected error happened: %v", msg.err)
