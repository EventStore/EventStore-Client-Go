...
func CreateTestClient(container *Container, t *testing.T) *esdb2.Client {
	config, err := esdb2.ParseConnectionString(fmt.Sprintf("esdb://admin:changeit@%s?tlsverifycert=false", container.Endpoint))
	if err != nil {
		t.Fatalf("Unexpected configuration error: %s", err.Error())
	}

	client, err := esdb2.NewClient(config)
	if err != nil {
...
func WaitForAdminToBeAvailable(t *testing.T, db *esdb2.Client) {
	count := 0
	...
	for count < 50 {
		...
		stream, err := db.ReadStream(ctx, "$users", esdb2.ReadStreamOptions{}, 1)
		...
		if err != nil {
			if esdbError, ok := esdb2.FromError(err); !ok {
				if esdbError.Code() == esdb2.ErrorResourceNotFound || esdbError.Code() == esdb2.ErrorUnauthenticated {
...
func WaitForLeaderToBeElected(t *testing.T, db *esdb2.Client) {
	count := 0
	...
	for count < 50 {
		...
		err := db.CreatePersistentSubscription(ctx, streamID, "group", esdb2.PersistentStreamSubscriptionOptions{})
...
func CreateClient(connStr string, t *testing.T) *esdb2.Client {
	config, err := esdb2.ParseConnectionString(connStr)
