...
func testValidClientCertificatesTlsWithRelativePath(t *testing.T, endpoint string) {
	...
	config, err := kurrent.ParseConnectionString(fmt.Sprintf("esdb://admin:changeit@%s?tls=true&tlscafile=%s&userCertFile=%s&userKeyFile=%s", endpoint, tlsCaFile, userCertFile, userKeyFile))
	if err != nil {
...
func testMissingCertificateFile(t *testing.T, endpoint string) {
	...
	config, err := kurrent.ParseConnectionString(fmt.Sprintf("esdb://admin:changeit@%s?tls=true&tlscafile=%s&usercertfile=%s", endpoint, tlsCaFile, userCertFile))

	_, err = kurrent.NewClient(config)
