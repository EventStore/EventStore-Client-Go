...
func GetClient(t *testing.T, container *Container) *kurrent.Client {
...
func CreateEmptyDatabase(t *testing.T) (*Container, *kurrent.Client) {
	return createDatabase(t, false)
}

func CreatePopulatedDatabase(t *testing.T) (*Container, *kurrent.Client) {
	return createDatabase(t, true)
}

func createDatabase(t *testing.T, populated bool) (*Container, *kurrent.Client) {
	isInsecure := GetEnvOrDefault("EVENTSTORE_INSECURE", "true") == "true"
...
func createTestClient(conn string, container *Container, t *testing.T) *kurrent.Client {
	config, err := kurrent.ParseConnectionString(conn)
	if err != nil {
		t.Fatalf("Unexpected configuration error: %s", err.Error())
	}

	client, err := kurrent.NewClient(config)
	if err != nil {
...
func WaitForAdminToBeAvailable(t *testing.T, db *kurrent.Client) {
	for count := 0; count < 50; count++ {
		ctx, cancel := context.WithTimeout(context.Background(), 1*time.Second)
		t.Logf("[debug] checking if admin user is available...%v/50", count)

		stream, err := db.ReadStream(ctx, "$users", kurrent.ReadStreamOptions{}, 1)
		...
		if err != nil {
			if esdbError, ok := kurrent.FromError(err); !ok {
				if esdbError.Code() == kurrent.ErrorCodeResourceNotFound ||
					esdbError.Code() == kurrent.ErrorCodeUnauthenticated ||
					esdbError.Code() == kurrent.ErrorCodeDeadlineExceeded ||
					esdbError.Code() == kurrent.ErrorUnavailable {
					time.Sleep(1 * time.Second)
...
func WaitForLeaderToBeElected(t *testing.T, db *kurrent.Client) {
	for count := 0; count < 50; count++ {
		streamID := NAME_GENERATOR.Generate()
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		t.Logf("[debug] checking if a leader has been elected...%v/50", count)

		err := db.CreatePersistentSubscription(ctx, streamID, "group", kurrent.PersistentStreamSubscriptionOptions{})
		...
		if err != nil {
			if esdbError, ok := kurrent.FromError(err); !ok {
				if esdbError.Code() == kurrent.ErrorCodeNotLeader || esdbError.Code() == kurrent.ErrorUnavailable || esdbError.Code() == kurrent.ErrorCodeUnauthenticated {
					time.Sleep(1 * time.Second)
					t.Logf("[debug] not available retrying...")
					cancel()
					continue
...
func CreateClient(connStr string, t *testing.T) *kurrent.Client {
	config, err := kurrent.ParseConnectionString(connStr)
